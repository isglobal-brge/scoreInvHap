...
`r Rpackage("snpfier")` deals with data either in PLINK format or as Bioconductor `VCF` class. In the case of having PLINK data, a list with two elements is required:

**NOTA: Carlos, normalmente la gente tiene el .fam, .ped, map files (o como se llamen)  y esto lo leen con read.plink, verdad?  entonces hay que partir de ahí ... si luego sólo le pasas el $genotype y el $map de ese objeto leído con read.plink hay que poner el código .. aunque no tengamos unos datos reales **
